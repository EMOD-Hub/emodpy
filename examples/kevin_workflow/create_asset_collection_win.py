...
if __name__ == "__main__":
    ...
    base_bamboo_url = "http://idm-bamboo:8085/artifact/DTKMASTER-DTKMARELWIN/shared/build-957/"
    # eradication bamboo path
    url = base_bamboo_url + "Eradication.exe/build/x64/Release/Eradication/Eradication.exe"
    # dll bamboo base path
    dll_base_url = base_bamboo_url + "Reporter-Plugins/build/x64/Release/reporter_plugins/"
    # dll list files
    dll_list = ["libReportAgeAtInfectionHistogram_plugin.dll", "libReportStrainTracking.dll"]

    # only download Eradication.exe if there is no on in local
    local_eradication_path = os.path.join('inputs', os_type, 'bin', 'Eradication.exe')
