...
class TestWorkflowDemographics(unittest.TestCase):
    ...
    def _migration_config_builder(self, config):
        """Config builder for migration tests: 20 timesteps."""
        config = self.builders.config_builder(config)
        config.parameters.Simulation_Duration = 20
        return config

    @staticmethod
    def _migration_report_builder(reporters):
        """Report builder that adds ReportHumanMigrationTracking."""
    ...
    def test_migration_gravity_local(self):
        """
            Testing that migration data generated by the gravity model can be written, added to
            demographics, and consumed by EMOD. Verifies Migration_Model, Enable_Local_Migration,
            and Migration_Pattern config params are set correctly.
        """
        from emodpy.migration.migration_data import MigrationData
        from emodpy.utils.emod_enum import MigrationType, MigrationPattern

        def build_demog():
            default_node = Node(lat=0, lon=0, pop=1000, forced_id=0)
            nodes = [Node(lat=0, lon=0, pop=10000, forced_id=1, name='NodeA'),
                     Node(lat=1, lon=1, pop=20000, forced_id=2, name='NodeB'),
                     Node(lat=2, lon=2, pop=15000, forced_id=3, name='NodeC')]
            demog = Demographics(nodes=nodes, default_node=default_node)
            data = MigrationData.from_gravity_model(demog, gravity_params=[1e-4, 1, 1, -1])
            demog.add_migration(data, migration_type=MigrationType.LOCAL,
                                migration_pattern=MigrationPattern.SINGLE_ROUND_TRIPS,
                                roundtrip_duration=1.0, roundtrip_probability=0.95)
            return demog

        task = EMODTask.from_defaults(eradication_path=self.builders.eradication_path,
                                      schema_path=self.builders.schema_path,
                                      config_builder=lambda cfg: self._migration_config_builder(cfg),
                                      demographics_builder=build_demog,
                                      report_builder=self._migration_report_builder)

        self.assertEqual(task.config['parameters']['Migration_Model'], "FIXED_RATE_MIGRATION")
        self.assertEqual(task.config['parameters']['Enable_Local_Migration'], 1)
        self.assertEqual(task.config['parameters']['Migration_Pattern'], "SINGLE_ROUND_TRIPS")
        self.assertEqual(task.config['parameters']['Local_Migration_Roundtrip_Duration'], 1.0)
        self.assertEqual(task.config['parameters']['Local_Migration_Roundtrip_Probability'], 0.95)
        self.assertEqual(task.config['parameters']['Simulation_Duration'], 20)
        self.run_exp(task)

    def test_migration_gravity_gender_with_heterogeneity(self):
        """
            Testing migration with ONE_FOR_EACH_GENDER data (female_multiplier), regional
            migration type with WAYPOINTS_HOME pattern, and migration heterogeneity
            (uniform distribution between 0.5 and 1.5).
        """
        from emodpy.migration.migration_data import MigrationData
        from emodpy.utils.emod_enum import MigrationType, MigrationPattern
        ...
        def build_demog():
            ...
            demog.add_migration(data, migration_type=MigrationType.REGIONAL,
                                migration_pattern=MigrationPattern.WAYPOINTS_HOME,
                                roundtrip_waypoints=5)
            demog.set_migration_heterogeneity_distribution(
                distribution=UniformDistribution(uniform_min=0.5, uniform_max=1.5))
            return demog
        ...
        self.assertEqual(task.config['parameters']['Migration_Model'], "FIXED_RATE_MIGRATION")
        self.assertEqual(task.config['parameters']['Enable_Regional_Migration'], 1)
        self.assertEqual(task.config['parameters']['Migration_Pattern'], "WAYPOINTS_HOME")
        self.assertEqual(task.config['parameters']['Roundtrip_Waypoints'], 5)
        self.assertEqual(task.config['parameters']['Enable_Migration_Heterogeneity'], 1)
        self.assertEqual(task.config['parameters']['Simulation_Duration'], 20)
        self.run_exp(task)

    def test_migration_multiple_types(self):
        """
            Testing that multiple migration types (LOCAL + REGIONAL) can be active simultaneously.
            Each type gets its own binary file, enable flag, and rate multiplier.
        """
        from emodpy.migration.migration_data import MigrationData
        from emodpy.utils.emod_enum import MigrationType, MigrationPattern
        ...
        def build_demog():
            ...
            local_data = MigrationData.from_gravity_model(demog, gravity_params=[1e-4, 1, 1, -1])
            demog.add_migration(local_data, migration_type=MigrationType.LOCAL,
                                migration_pattern=MigrationPattern.SINGLE_ROUND_TRIPS,
                                roundtrip_duration=1.0, roundtrip_probability=0.95,
                                x_modifier=2.0)

            regional_data = MigrationData.from_gravity_model(demog, gravity_params=[5e-5, 1, 1, -1])
            demog.add_migration(regional_data, migration_type=MigrationType.REGIONAL,
                                migration_pattern=MigrationPattern.SINGLE_ROUND_TRIPS,
                                roundtrip_duration=5.0, roundtrip_probability=0.5)
        ...
        self.assertEqual(task.config['parameters']['Enable_Local_Migration'], 1)
        self.assertEqual(task.config['parameters']['Enable_Regional_Migration'], 1)
        self.assertEqual(task.config['parameters']['x_Local_Migration'], 2.0)
        self.assertEqual(task.config['parameters']['Simulation_Duration'], 20)
        self.run_exp(task)

    def test_migration_from_csv(self):
        """
            Testing that migration data loaded from a CSV file can be assigned to a migration type
            and consumed by EMOD.
        """
        import tempfile
        from emodpy.migration.migration_data import MigrationData
        from emodpy.utils.emod_enum import MigrationType, MigrationPattern
        ...
        def build_demog():
            ...
            csv_content = "source_node,destination_node,rate\n1,2,0.01\n2,1,0.005\n"
            csv_path = os.path.join(tempfile.gettempdir(), "test_mig.csv")
            with open(csv_path, 'w') as f:
                f.write(csv_content)

            data = MigrationData.from_csv(csv_path, idref=demog.idref)
            demog.add_migration(data, migration_type=MigrationType.LOCAL,
                                migration_pattern=MigrationPattern.SINGLE_ROUND_TRIPS,
                                roundtrip_duration=1.0, roundtrip_probability=0.9)
            return demog
    ...
    def test_migration_age_dependent(self):
        """
            Testing age-dependent migration rates using apply_modifier. Creates different migration
            rates for children (<15) and adults (>=15).
        """
        from emodpy.migration.migration_data import MigrationData
        ...
        def build_demog():
            ...
            base = MigrationData.from_gravity_model(demog, gravity_params=[1e-4, 1, 1, -1])
            modified = base.apply_modifier(
                ages=[0, 15, 65],
                modifier_fn=lambda rate, age, gender: rate * 0.5 if age < 15 else rate
            )
            demog.add_migration(modified, migration_type=MigrationType.LOCAL,
                                interpolation_type=InterpolationType.PIECEWISE_CONSTANT,
                                migration_pattern=MigrationPattern.SINGLE_ROUND_TRIPS,
                                roundtrip_duration=1.0, roundtrip_probability=0.95)
            return demog
...
class TestWorkflowDemographicsGeneric(TestWorkflowDemographics):
    ...
    test_migration_gravity_local = None
    test_migration_gravity_gender_with_heterogeneity = None
    test_migration_multiple_types = None
    test_migration_from_csv = None
    test_migration_age_dependent = None
