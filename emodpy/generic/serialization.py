...
def enable_serialization(task: 'EMODTask', use_absolute_times: bool = False):
    """
    Enable serialization either by TIME or TIMESTEP based on use_absolute_times.
    Args:
        task (EMODTask): Task to enable
        use_absolute_times (bool): When true, **Serialization_Type** will be set to TIME, otherwise it will be set to
        TIMESTEP.
    """
...
def add_serialization_timesteps(task: EMODTask, timesteps: List[int], end_at_final: bool = False, use_absolute_times: bool = False):
    ...
    Args:
        ...
        timesteps (List[int]): Array of integers representing the time steps to use
            end_at_final (bool): False means set the simulation duration such that the last serialized_population
            file ends the simulation. NOTE: may not work if time step size is not 1.
        use_absolute_times (bool): False means the method will define simulation times instead of time steps
            see documentation on Serialization_Type for details.
    """
